...
public class JdbcVisitRepositoryImpl implements VisitRepository {
	...
	public Collection<Visit> findAllVisits() throws DataAccessException {
		final List<Pet> pets = this.jdbcTemplate.query("SELECT id, name, owner_id, type_id FROM pets",
				BeanPropertyRowMapper.newInstance(Pet.class));
		final List<PetType> PetTypes = this.jdbcTemplate.query("SELECT id, name FROM types",
				BeanPropertyRowMapper.newInstance(PetType.class));
		final List<Owner> owners = this.jdbcTemplate.query("SELECT id, first_name, last_name, telephone FROM owners",
				BeanPropertyRowMapper.newInstance(Owner.class));
		for (Pet pet : pets) {
			final List<Integer> ownerids = this.jdbcTemplate.query("SELECT owner_id FROM pets WHERE id=?",
			...
			for (int ownerid : ownerids) {
				Owner owner = EntityUtils.getById(owners, Owner.class, ownerid);
				pet.setOwner(owner);
			}
			final List<Integer> pettypes = this.jdbcTemplate.query("SELECT type_id FROM pets WHERE id=?",
					new BeanPropertyRowMapper<Integer>() {
						@Override
						public Integer mapRow(ResultSet rs, int row) throws SQLException {
							return Integer.valueOf(rs.getInt(1));
			...
			for (int pettype : pettypes) {
				PetType pt = EntityUtils.getById(PetTypes, PetType.class, pettype);
				pet.setType(pt);
			}
		}
		...
		final List<Visit> visits = this.jdbcTemplate.query(
				...
				new BeanPropertyRowMapper<Visit>() {
					...
					public Visit mapRow(ResultSet rs, int row) throws SQLException {
						Visit visit = new Visit();
						visit.setId(rs.getInt("id"));
						Pet pet = EntityUtils.getById(pets, Pet.class, rs.getInt("pet_id"));
						visit.setPet(pet);
						java.sql.Date dates = rs.getDate("visit_date");
						DateTime date = new DateTime(dates);
						visit.setDate(date.minusDays(1));
