...
public abstract class AbstractClinicServiceTests {
	...
	public void OwnerToString() {
		...
		assertThat(owner.toString().length()).isGreaterThan(0);
	}
	...
	public void shouldFindPet() {
		Owner owner = this.clinicService.findOwnerById(1);
		assertThat(owner.getPet("Leo", true)).isNotNull();
		assertThat(owner.getPets().size()).isEqualTo(1);
	}
	...
	@Test
	public void shouldFindVisits() {
		Collection<Visit> visits = this.clinicService.findVisits();
		Visit visit = EntityUtils.getById(visits, Visit.class, 3);
		DateTime dt = new DateTime("2013-01-03");
		dt = dt.dayOfYear().roundFloorCopy().minusDays(1);
	...
	public void shouldFindSupplies() {
		Collection<Supply> supplies = this.clinicService.findSupplies();
		Supply supply = EntityUtils.getById(supplies, Supply.class, 3);
	...
	@Test
	public void shouldFindSupplyWithCorrectId() {
		Supply supply = this.clinicService.findSupplyById(1);
		assertThat(supply.getItem()).isEqualTo("dog food");
	}

	@Test
	@Transactional
	public void shouldDiagnosePet() {
		Visit visit = this.clinicService.findVisitById(1);
		visit.setDiagnosis("Sick");
		this.clinicService.saveVisit(visit);
		visit = this.clinicService.findVisitById(1);
		assertThat(visit.getDiagnosis()).isEqualTo("Sick");
	}

	@Test
	@Transactional
	public void shouldEditSupply() {
		Supply supply = this.clinicService.findSupplyById(1);
		supply.setItem("food");
		supply.setOrdered(2);
		this.clinicService.saveSupply(supply);
		supply = this.clinicService.findSupplyById(1);
		assertThat(supply.getItem()).isEqualTo("food");
		assertThat(supply.getOrdered()).isEqualTo(2);
	}

	@Test
	@Transactional
	public void shouldAddSupply() {
		Supply supply = new Supply();
		supply.setItem("food");
		this.clinicService.saveSupply(supply);
		supply = this.clinicService.findSupplyById(supply.getId());
		assertThat(supply.getItem()).isEqualTo("food");
	}

	@Test
	@Transactional
	public void unassignVet() {
		Visit visit = this.clinicService.findVisitById(1);
		this.clinicService.unassignVet(visit.getId());
		visit = this.clinicService.findVisitById(1);
		assertThat(visit.getVet()).isEqualTo("");
	}

	@Test
	public void billOwner() {
		Visit visit = this.clinicService.findVisitById(1);
		Date d = new Date();
		DateTime dt = new DateTime(d);
		DateTime date = visit.getDate();
		int x = dt.compareTo(date);
		int y = date.compareTo(dt);
		int z = date.plusDays(16).compareTo(dt);
	...
	public void markPaid() {
		Visit visit = this.clinicService.findVisitById(2);
		this.clinicService.markPaid(visit.getId(), true);
		visit = this.clinicService.findVisitById(2);
		assertThat(visit.getPaid()).isEqualTo(true);
	}
}